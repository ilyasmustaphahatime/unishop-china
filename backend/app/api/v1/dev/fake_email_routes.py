import ipaddress
import math
from datetime import datetime

from fastapi import APIRouter, Depends, HTTPException, Request, Response, status
from pydantic import BaseModel, ConfigDict

from app.api.v1.auth.dependencies import get_current_user
from app.integrations.email_verification_delivery import (
    DevelopmentFakeEmailVerificationStore,
)
from app.models.base import utc_now
from app.services.auth_service import SafeAuthenticatedUser

NO_STORE_HEADERS = {"Cache-Control": "no-store", "Pragma": "no-cache"}


class DevelopmentFakeEmailVerificationResponse(BaseModel):
    model_config = ConfigDict(extra="forbid")

    message_id: str
    code: str
    created_at: datetime
    expires_at: datetime
    expires_in_seconds: int


def _require_loopback(request: Request) -> None:
    host = request.client.host if request.client is not None else ""
    try:
        allowed = ipaddress.ip_address(host).is_loopback
    except ValueError:
        allowed = False
    if not allowed:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail={"code": "LOCALHOST_REQUIRED", "message": "This route is local-only."},
            headers=NO_STORE_HEADERS,
        )


def create_development_fake_email_router(
    store: DevelopmentFakeEmailVerificationStore,
) -> APIRouter:
    router = APIRouter(tags=["development-fake-email-verification"])

    @router.get("/latest", response_model=DevelopmentFakeEmailVerificationResponse)
    def latest_fake_email(
        request: Request,
        response: Response,
        current_user: SafeAuthenticatedUser = Depends(get_current_user),
    ) -> DevelopmentFakeEmailVerificationResponse:
        _require_loopback(request)
        if request.query_params:
            raise HTTPException(
                status_code=status.HTTP_422_UNPROCESSABLE_CONTENT,
                detail={"code": "INVALID_QUERY", "message": "Unknown query parameter."},
                headers=NO_STORE_HEADERS,
            )
        message = store.latest_available(current_user.id)
        if message is None:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND,
                detail={"code": "MESSAGE_NOT_AVAILABLE", "message": "No message is available."},
                headers=NO_STORE_HEADERS,
            )
        response.headers.update(NO_STORE_HEADERS)
        return DevelopmentFakeEmailVerificationResponse(
            message_id=message.message_id,
            code=message.code,
            created_at=message.created_at,
            expires_at=message.expires_at,
            expires_in_seconds=max(
                0,
                math.ceil((message.expires_at - utc_now()).total_seconds()),
            ),
        )

    @router.delete("/{message_id}", status_code=status.HTTP_204_NO_CONTENT)
    def consume_fake_email(
        message_id: str,
        request: Request,
        response: Response,
        current_user: SafeAuthenticatedUser = Depends(get_current_user),
    ) -> None:
        _require_loopback(request)
        store.consume_message(user_id=current_user.id, message_id=message_id)
        response.headers.update(NO_STORE_HEADERS)

    return router
